...
def GRAMPERSECFLOWSENSOR(messages):
    # decode the twos complement
    d = messages[0].data[2:]
    a = twos_comp(d[0], 8)
    bits = BitArray(d)
    if int(bits[0]) == 1:
        ...
        c = twos_comp(d[2], 8)
        sensor1 =((256 * b) + c) /32
        sensor1=sensor1 * Unit.gps,  # sensor 1
    if int(bits[1]) == 1:
        d = twos_comp(d[3], 8)
        e = twos_comp(d[4], 8)
        sensor2 = ((256 * d) + e) / 32
        sensor2=sensor2 * Unit.gps  # sensor 2
    return (
        sensor1,
        sensor2
...
def tempmulti(messages):
    d = messages[0].data[2:]
    a = twos_comp(d[0], 8)
    bits = BitArray(d)
    if int(bits[0]) == 1:
        b = twos_comp(d[1], 8)
        b1= Unit.Quantity(b - 40, Unit.celsius)  # sensor 1
    if int(bits[1]) == 1:
        c = twos_comp(d[2], 8)
        c1 = Unit.Quantity(c-40, Unit.celsius)  # sensor 2
    return (
        b1,
        c1
